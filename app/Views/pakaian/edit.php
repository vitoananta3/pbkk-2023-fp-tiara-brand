    ...
    <div class="flex items-center w-6/12 max-w-screen-xl mb-8">
        <div class="flex flex-col w-full">
            ...
            <div class="text-sm flex flex-col justify-center gap-2">
                ...
                <div class="my-2"></div>
                <div class="relative">
                    <input type="text" id="floating_outlined" class="border block px-4 pb-3 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-[#434C5E] dark:border-<?= session('validation_errors') && array_key_exists('ukuran', session('validation_errors')) && session('validation_errors')['ukuran'] ? 'red' : '[#434C5E]' ?> dark:focus:border-<?= session('validation_errors') && array_key_exists('ukuran', session('validation_errors')) && session('validation_errors')['ukuran'] ? 'red' : 'blue' ?> focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="..." autofocus name="ukuran" value="<?= (old('ukuran')) ? old('ukuran') : $pakaian['ukuran'] ?>" />
                    <label for="ukuran" class="absolute text-[#434C5E] dark:text-[#434C5E] duration-300 transform -translate-y-4 scale-90 top-2 z-10 origin-[0] bg-[#E5E9F0] dark:bg-[#E5E9F0] px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-<?= session('validation_errors') && array_key_exists('ukuran', session('validation_errors')) && session('validation_errors')['ukuran'] ? 'red' : 'blue' ?> peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-90 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Ukuran</label>
                </div>
                <div class="text-red-600"><?php
                ...
                <div class="my-2"></div>
                <div class="relative">
                    <textarea type="text" id="floating_outlined" class="h-32 border block px-4 pb-3 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-[#434C5E] dark:border-<?= session('validation_errors') && array_key_exists('deskripsi', session('validation_errors')) && session('validation_errors')['deskripsi'] ? 'red-600' : '[#434C5E]' ?> dark:focus:border-<?= session('validation_errors') && array_key_exists('deskripsi', session('validation_errors')) && session('validation_errors')['deskripsi'] ? 'red-600' : 'blue-500' ?> focus:outline-none focus:ring-0 focus:border-blue-600 peer" autofocus name="deskripsi"><?= (old('deskripsi')) ? old('deskripsi') : $pakaian['deskripsi'] ?></textarea>
                    <label for="deskripsi" class="absolute text-[#434C5E] dark:text-[#434C5E] duration-300 transform -translate-y-4 scale-90 top-2 z-10 origin-[0] bg-[#E5E9F0] dark:bg-[#E5E9F0] px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-<?= session('validation_errors') && array_key_exists('deskripsi', session('validation_errors')) && session('validation_errors')['deskripsi'] ? 'red-600' : 'blue-500' ?> peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-90 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Deskripsi</label>
                </div>
                <div class="text-red-600"><?php
                ...
                <div class="relative">
                    <input type="text" id="floating_outlined" class="border block px-4 pb-3 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-[#434C5E] dark:border-<?= session('validation_errors') && array_key_exists('harga', session('validation_errors')) && session('validation_errors')['harga'] ? 'red-600' : '[#434C5E]' ?> dark:focus:border-<?= session('validation_errors') && array_key_exists('harga', session('validation_errors')) && session('validation_errors')['harga'] ? 'red-600' : 'blue-500' ?> focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" autofocus name="harga" value="<?= (old('harga')) ? old('harga') : $pakaian['harga'] ?>"></input>
                    <label for="harga" class="absolute text-[#434C5E] dark:text-[#434C5E] duration-300 transform -translate-y-4 scale-90 top-2 z-10 origin-[0] bg-[#E5E9F0] dark:bg-[#E5E9F0] px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-<?= session('validation_errors') && array_key_exists('harga', session('validation_errors')) && session('validation_errors')['harga'] ? 'red-600' : 'blue-500' ?> peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-90 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Harga (dalam Rp)</label>
                </div>
                <div class="text-red-600"><?php
                ...
                <div class="my-2"></div>
                <div class="relative">
                    <input type="text" id="floating_outlined" class="border block px-4 pb-3 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-[#434C5E] dark:border-<?= session('validation_errors') && array_key_exists('stok', session('validation_errors')) && session('validation_errors')['stok'] ? 'red-600' : '[#434C5E]' ?> dark:focus:border-<?= session('validation_errors') && array_key_exists('stok', session('validation_errors')) && session('validation_errors')['stok'] ? 'red-600' : 'blue-500' ?> focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" autofocus name="stok" value="<?= (old('stok')) ? old('stok') : $pakaian['stok'] ?>"></input>
                ...
                <img class="card-image-preview img-thumbnail img-preview" src="<?= base_url(); ?>assets/gambar-pakaian/<?= $pakaian['gambar']; ?>" alt="preview-gambar-pakaian">
                ...
                <p class="mt-1 text-<?= session('validation_errors') && array_key_exists('gambar', session('validation_errors')) && session('validation_errors')['gambar'] ? 'red-600' : '[#434C5E]' ?> font-thin">.jpg, .jpeg or .png | max size 1MB</p>
                <div class="text-red-600"><?php
            ...
            <div class="my-8"></div>
            <?= form_submit('submit', 'Edit Pakaian', ['class' => 'bg-[#434C5E] hover:bg-[#81A1C1] text-[#E5E9F0] hover:text-[#434C5E] rounded-md w-full px-5 py-2.5 transition-colors duration-200 border-2 border-black']) ?>
            <?= form_close() ?>
            <div class="my-8">
                <div class="flex justify-end w-full">
                    <a href="/pakaian" class="text-sm border border-[#434C5E] hover:bg-[#81A1C1] text-[#434C5E] hover:text-[#434C5E] rounded-md px-5 py-2.5 transition-colors duration-200">
